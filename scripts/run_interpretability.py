...
model_class = [Ridge,  XGBRegressor, CatBoostRegressor, [XGBRegressor, Ridge, CatBoostRegressor]] 
...
input_path = "raw_results3"
output_path = "raw_interpretability_results3"
...
for key in features_dict.keys():
    ...
    # Just to obtain selected features. 
    results = load_results_from_json(f"{input_path}/XGBRegressor/tuned/{key}.json")
...
for model in model_class:
    ...
    for key in ['trainTest', 'trainTest_removed', 'randomCV', 'randomCV_removed', 'groupedCV', 'groupedCV_removed']:
        if isinstance(model, list):
            ...
            for m in model:
                m_name = m.__name__
                results = load_results_from_json(f"{input_path}/{m_name}/tuned/{key}.json")
                params.append(results['params'])
            params_dict[key] = params

        else:
            results = load_results_from_json(f"{input_path}/{model_name}/tuned/{key}.json")
            params_dict[key] = results['params']

    ### Permutation Importace

    # Train Test
    pi_trainTest = train_test_interpretability(X, y, model, random_cv_splitter, method="permutation", 
                                               features_to_use = features_dict['trainTest'], params = params_dict["trainTest"])
    pi_trainTest_removed = train_test_interpretability(X_removed, y_removed, model, random_cv_splitter, method="permutation", 
    ...
    # Train Test
    shap_trainTest = train_test_interpretability(X, y, model, random_cv_splitter, method="shap", shap_background_size=100,
                                               features_to_use = features_dict['trainTest'], params = params_dict["trainTest"])
    shap_trainTest_removed = train_test_interpretability(X_removed, y_removed, model, random_cv_splitter, method="shap", shap_background_size=100,
                                               features_to_use = features_dict['trainTest_removed'], params = params_dict["trainTest_removed"])
    # Random CV
